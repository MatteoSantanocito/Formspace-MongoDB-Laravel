...
function deletePost(event){
    ...
    fetch("/profile/deletePost/"+encodeURIComponent(post.dataset.id))
}

fetch("/post/get/").then(onResponse).then(createPost)
...
function createPost(json){
    ...
    for(let i of json){
        ...
        const upper_section = createUpperSection(i.title, i.username, i.created_at.slice(0,10)) //per laravel bisogna usare created_at invece del published_at che avevo impostato precedentemente
